...
function create_elements(req) {
  if (req.status === 'pending') {
    ...
    parent.addEventListener('click', () => editRequest(req.request_id));
    ...
    ref.textContent = `Ref: PND-${req.request_id}`;
    ...
    pendingRequests.appendChild(parent);
  }
...
async function editRequest(id) {
  ...
  if (user_role === 'resident') {
    window.location.assign(`viewEditRequest.html?id=${id}`);
  } else if (user_role === 'council') {
    window.location.assign(`viewRequest.html`);
  }
}

function viewedRequests(id) {
